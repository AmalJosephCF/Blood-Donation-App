...
def donate_blood_view(request):
    ...
    if request.method == 'POST':
        ...
        if donation_form.is_valid():
            donor = models.Donor.objects.get(user_id=request.user.id)
            last_donation = models.BloodDonate.objects.filter(donor=donor).order_by('-date').first()
            if last_donation:
                last_donation_date = last_donation.date
                six_months_ago = datetime.now() - timedelta(days=180)
                if isinstance(last_donation_date, datetime):  # Check if last_donation_date is datetime
                    last_donation_date = last_donation_date.date()  # Convert datetime to date
                if last_donation_date > six_months_ago.date():  # Ensure six_months_ago is a date object
                    # If last donation was within the last 6 months, show a popup
                    context = {'last_donation_date': last_donation_date}
                    popup_html = render_to_string('donor/popup.html', context=context, request=request)
